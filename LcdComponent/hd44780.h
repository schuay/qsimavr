...
struct hd44780_t;
typedef void (*display_change_hook_t)(void *instance, const struct hd44780_t *vram);
...
typedef struct hd44780_t
{
    ...
    uint16_t cursor;            // offset in vram. 0x00 - 0x27 represent the first line,
                                // 0x28 - 0x49 the second line. This can be a little confusing,
                                // because in vram the second line starts at 0x40, not 0x28.
    uint8_t shift;              // Shift count. Represents the address displayed at
                                // the top left in the first line. Valid values are
                                // 0x00 to 0x27.
    uint8_t  vram[80 + 64];
    ...
    /**
     * These variables were added to facilitate event-based communication
     * with C++ classes. We need to store a pointer to the attached class instance,
     * and a pointer to the function to call.
     */
    void * instance;
    display_change_hook_t display_change_hook;
} hd44780_t;
