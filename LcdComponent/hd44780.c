        ...
        hd44780_t *b)
{
    ...
    b->display_change_hook(b->instance, b);
    ...
    switch (top) {
        ...
        case 4: {   // 0 0 0 1 S/C R/L x x
            ...
            if (b->datapins & (1 << 3)) { /* Display shift. */
                ...
                if (!right && b->cursor == HD44780_ROW1_START) {
                    b->cursor = HD44780_ROW2_START + HD44780_ROW_LENGTH - 1;
                } else if (!right && b->cursor == HD44780_ROW2_START) {
                    b->cursor = HD44780_ROW1_START + HD44780_ROW_LENGTH - 1;
                } else if (right && b->cursor == HD44780_ROW1_START + HD44780_ROW_LENGTH - 1) {
                    b->cursor = HD44780_ROW2_START;
                } else if (right && b->cursor == HD44780_ROW2_START + HD44780_ROW_LENGTH - 1) {
                    b->cursor = HD44780_ROW1_START;
                } else {
                    b->cursor += offset;
                }
                avr_raise_irq(b->irq + IRQ_HD44780_ADDR, b->cursor);
        ...
        display_change_hook_t display_change_hook)
{
    ...
    b->h = height;
    b->instance = instance;
    b->display_change_hook = display_change_hook;
